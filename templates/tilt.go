...
local_resource(
    ...
    cmd   ='cd app-api && go mod tidy && go build -o ../tmp/{{.AppName}}-local ./main && cd ..',
    deps  =['app-api', '../goald'], # taking into account the dependencies
    ignore=['app-api/go.sum', 'app-api/config.yaml'],
    )
...
#     dir          ='app-web',
#     cmd          =webDepsCmd,
#     deps         =['app-web/package.json'],
#     serve_cmd    ='LOCAL_API_URL=http://'+apiHost+':24243 npm run dev',
