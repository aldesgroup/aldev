...
const TiltfileAPI = `# Generated by aldev, do not edit!
# not sending data
...
config.define_bool('verbose', usage='use this to activate the verbose mode where it can apply')
cfg = config.parse()
...
local_resource(
    ...
    cmd   ='aldev codegen' + codeGenMode,
    # taking into account the dependencies
    deps  =['{{.API.SrcDir}}', '../goald', '../emerald'], # TODO: REMOVE EMERALD
...
const TiltfileNativePart = `

# --- NATIVE part ----------------------------------------------------------------

local_resource(
    name         ='{{.AppName}}-native-run',
    dir          ='{{.Native.SrcDir}}',
    cmd          ='npx react-native run-windows --no-telemetry',
    resource_deps=['{{.AppName}}-lb-port-forward'],
    )
` // end of template
