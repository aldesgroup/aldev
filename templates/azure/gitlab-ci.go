...
# Writes the deploy token to a temp file consumed by --mount=type=secret in the build.
...
#   needs: [ mark_qua_lkg ]
#   script:
#   - az account set --subscription "subs-pdt-prd"
#   - export ENV="prd"
#   - export ACA_RG="rg-pdt-${ENV}-${APP}"
#   - export ACA_NAME="aca-pdt-${ENV}-${APP}"
#   - export IMAGE_TAG="${CI_COMMIT_TAG}"
#   - export IMAGE="crpdt.azurecr.io/${IMAGE_REPO}:${IMAGE_TAG}"
#   - *deploy_aca_image
#   environment:
#     name: prd
#   when: manual
...
#       echo "ERROR: set ROLLBACK_ENV (dev|qua|prd) and ROLLBACK_TAG (e.g. qua-lkg, prd-lkg, v1.2.3)"
#       exit 1
#     fi

#   - az account set --subscription "subs-pdt-${ROLLBACK_ENV}"
#   - export ENV="$ROLLBACK_ENV"
#   - export ACA_RG="rg-pdt-${ENV}-${APP}"
#   - export ACA_NAME="aca-pdt-${ENV}-${APP}"
#   - export IMAGE="crpdt.azurecr.io/${IMAGE_REPO}:${ROLLBACK_TAG}"
#   - *deploy_aca_image
#   environment:
#     name: rollback
#   when: manual
